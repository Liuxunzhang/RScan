//! Redis unauthorized / weak-password detection and optional write-exploit plugin.

use crate::connection::{connect_stream, read_available, send_tcp_command, write_and_flush};
use crate::credentials::passwords_for_user;
use crate::runtime::{RedisRuntimeOptions, brute_force_disabled, current_redis_runtime_options};
...
fn redis_run_exploit(
    ...
    if let Some(key_file) = options.redis_file.as_ref()
        && let Ok(key) = redis_read_first_nonempty_line(key_file)
    {
