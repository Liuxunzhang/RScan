//! SSH weak-password / key-authentication detection plugin.

use crate::connection::connect_stream;
...
pub(crate) fn scan_ssh(
    ...
    for username in usernames_for_service("ssh", context) {
        if let Some(key_path) = &context.ssh_key_path
            && ssh_key_login(target, &username, key_path, context.timeout_secs)?
        {
