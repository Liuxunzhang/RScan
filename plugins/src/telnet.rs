...
pub(crate) fn scan_telnet(target: &OpenService, context: &PluginContext) -> Result<Option<PluginFinding>> {
...
pub(crate) fn probe_telnet_access(target: &OpenService, timeout_secs: u64) -> Result<TelnetAccess> {
...
pub(crate) fn looks_like_login_prompt(value: &str) -> bool {
...
pub(crate) fn looks_like_password_prompt(value: &str) -> bool {
    value.to_ascii_lowercase().contains("password:")
}

pub(crate) fn contains_auth_failure(value: &str) -> bool {
    let value = value.to_ascii_lowercase();
...
pub(crate) fn looks_like_shell_prompt(value: &str) -> bool {
