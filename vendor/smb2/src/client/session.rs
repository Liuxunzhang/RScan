...
impl Session {
    ...
    pub async fn setup(
        ...
        Self::setup_with_nt_hash(conn, username, password, domain, None).await
    }

    /// Perform session setup with either a plaintext password or precomputed NT hash.
    pub async fn setup_with_nt_hash(
        conn: &mut Connection,
        username: &str,
        password: &str,
        domain: &str,
