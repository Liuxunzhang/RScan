...
pub struct ClientConfig {
    ...
    pub password: String,
    /// Optional precomputed NT hash used instead of the plaintext password.
...
impl SmbClient {
    ...
    pub async fn connect(config: ClientConfig) -> Result<Self> {
        ...
        let session = Session::setup_with_nt_hash(
            &mut conn,
            &config.username,
            &config.password,
            &config.domain,
            config.nt_hash.clone(),
        )
        .await?;
